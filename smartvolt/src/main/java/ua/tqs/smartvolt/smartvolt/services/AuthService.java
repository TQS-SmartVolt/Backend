...
public class AuthService {
  ...
  public User createUser(UserSignUp userSignUp) throws ResourceNotFoundException {
    ...
    if (existingUser.isPresent()) {
      throw new ResourceNotFoundException(
          "User with email " + userSignUp.getEmail() + " already exists");
    }
  ...
  public AuthResponse signIn(AuthRequest authRequest) throws ResourceNotFoundException {
    ...
    user = evDriverService.getEvDriverByEmail(currentEmail).orElse(null);

    if (user == null) {
      user = stationOperatorService.getStationOperatorByEmail(currentEmail).orElse(null);
    }
    ...
    String token = jwtTokenUtil.generateToken(user.getUserId(), user.getRoles());
  ...
  public AuthResponse me() throws ResourceNotFoundException {
    ...
    user = evDriverService.getEvDriverById(currentId).orElse(null);

    if (user == null) {
      user = stationOperatorService.getStationOperatorById(currentId).orElse(null);
    }
